...
export const bot_response=async(req,res)=>{
  ...
  "Authorization": `Bearer ${process.env.GROQ_API_KEY}`
       ...
       const groqtit=await axios.post("https://api.groq.com/openai/v1/chat/completions",
        {
        model:"llama-3.1-8b-instant",
        messages:[
            {
                role:'system',
                content:`Generate a title for this response so I can use it in the sidebar. Here is the response: ${groqres.data.choices[0].message.content}`
            },
            {
                role:'user',
  ...
  "Authorization": `Bearer ${process.env.GROQ_API_KEY}`
      ...
      const response=groqtit.data.choices[0].message.content
      const match = response.match(/\*\*(.*?)\*\*/);
      const groqtitle = match ? match[1] : "Untitled Response";

    
        
        const storeresponses=await store_responses.create({
          userInput:input,botresponse:groqres.data.choices[0].message.content,title:groqtitle
        })
          res.status(200).json(storeresponses)
  } catch (error) {
    console.log(error)
  }
}
...
export const deletechat=async(req,res)=>{
  try {
    ...
    const deletethechat=await store_responses.deleteMany({_id:new ObjectId(req.params.id)})
